...
function buildComponentDefinition(name, dashCaseName, props, functions, defaultValues, componentDescription, checker) {
    ...
    return {
        ...
        functions: functions.map((func) => ({
            ...
            returnType: (0, type_utils_1.stringifyType)(func.rawType.getNonNullableType().getCallSignatures()[0].getReturnType(), checker),
        ...
        properties: onlyProps.map((property) => {
            const { type, inlineType } = (0, object_definition_1.getObjectDefinition)(property.type, property.rawType, property.rawTypeNode, checker);
            return {
                name: property.name,
                type: type,
        ...
        events: events.map((event) => {
            const { detailType, detailInlineType, cancelable } = extractEventDetails(event.rawType, event.rawTypeNode, checker);
            return {
                name: event.name,
...
function extractEventDetails(type, typeNode, checker) {
    ...
    if (detailType && detailType.getProperties().length > 0) {
        const { type, inlineType } = (0, object_definition_1.getObjectDefinition)((0, type_utils_1.stringifyType)(detailType, checker), detailType, typeNode, checker);
        return {
            detailType: type,
